...
{
    public class ExpenseTypeFieldService : IExpenseTypeFieldService
    {
        public string controller_key { get; set; }
        private IMessageDialog _dialog;
        private dto_logged_user m_logged_user { get; set; }

        string table_name =  FleetManager.DbBase.tableNames.exp_type_field_tb;
        public ExpenseTypeFieldService(IMessageDialog dialog, ILoggedInUserService l_user)
        {
            _dialog = dialog;
        ...
        public Task<bool> Delete(int id)
        {
            ...
            try
            {
                using (var _db = fnn.GetDbContext())
                {
                    var exp_type_field = _db.EXPENSE_TYPE_FIELDS.Where(e => e.et_field_type_id == id & e.delete_id == 0).FirstOrDefault();
                    if (exp_type_field == null)
                    {
                        AddErrorMessage("Error", "Error", "Unable To Find Expense Type Field Object");
                        _record_deleted = false;
                        return Task.FromResult(_record_deleted);
                    }
                    else
                    {
                        var _has_dependency = DbHelper.HasDbDependencies(_db.Database, new string[] { DbHelper.GetTableSchemaName( FleetManager.DbBase.tableNames.exp_type_field_tb) }, DbHelper.GetDbSchema(), new string[] { "et_field_id" }, id);
                        ...
                        else
                        {
                            var _result = DbHelper.DeleteRecordWithDeleteId(new DbHelperDeleteRecordC()
                            {
                                ...
                                pk_id = exp_type_field.et_field_id,
                                table_name = DbHelper.GetTableSchemaName( FleetManager.DbBase.tableNames.exp_type_field_tb)
                            }, _db);
        ...
        public Task<List<vh_expense_type_fieldC>> GetAll(long fs_timestamp)
        {
            List<vh_expense_type_fieldC> _list = null;
            string _sql = null;
            try
            {
                using (var _db = fnn.GetDbConnection())
                {
                    if (fs_timestamp == 0)
                    {
                        ...
                        _list = _db.Query<vh_expense_type_fieldC>(_sql).ToList();
                    }
                    else
                    {
