...
namespace MutticoFleet.Services
{
    public class VehicleOwnerService : IVehicleOwnerService
    {
        ...
        public string _table_name =  FleetManager.DbBase.tableNames.vehicle_owner_tb;
        ...
        public Task<List<vehicle_ownerC>> GetAll(long fs_timestamp)
        {
            List<vehicle_ownerC> _list = null;
            string _sql = null;
            try
            {
                using (var _db = fnn.GetDbConnection())
                {
                    if (fs_timestamp == 0)
                    {
                        ...
                        _list = _db.Query<vehicle_ownerC>(_sql).ToList();
                    }
                    else
                    {
